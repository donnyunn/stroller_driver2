...
#define SAMPLE_DEVICE_NAME          "INNOM"    //The Device Name Characteristics in GAP
...
static ble_spp_server_t * this = NULL;
...
void uart_debug(char * data, uint16_t len);
...
{
    char * monitor = NULL;
    monitor = (char*)malloc(len*2 + 1);
    for (int i = 0; i < len; i++) {
        sprintf(monitor+(2*i), "%02x", data[i]);
    }
    ESP_LOGI(GATTS_TABLE_TAG, "%s", monitor);
    free(monitor);
}
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    uint8_t * spp_data_buff = NULL;
                    spp_data_buff = (uint8_t * )malloc((spp_mtu_size - 3) * sizeof(uint8_t));
                    if (spp_data_buff == NULL) {
                        ESP_LOGE(GATTS_TABLE_TAG, "%s malloc failed", __func__);
                        break;
                    }
                    memset(spp_data_buff, 0x0, (spp_mtu_size - 3));
                    memcpy(spp_data_buff, p_data->write.value, p_data->write.len);
                    xQueueSend(this->data_queue, &spp_data_buff, 10/portTICK_PERIOD_MS);
#ifdef SPP_DEBUG_MODE
                    esp_log_buffer_char(GATTS_TABLE_TAG,(char *)(p_data->write.value),p_data->write.len);
#else
                    // uart_debug((char *)(p_data->write.value), p_data->write.len);
...
void ble_spp_server_init(ble_spp_server_t * spp)
{
    ...
    this = spp;
    this->data_queue = xQueueCreate(10, sizeof(uint32_t));
